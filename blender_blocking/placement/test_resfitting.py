...
def generate_cylinder_points(radius=1.0, height=2.0, num_points=1000):
...
def generate_cone_points(radius_bottom=1.5, radius_top=0.0, height=2.0, num_points=1000):
...
def test_initialization_from_slices():
    ...
    assert all(isinstance(sf, SuperFrustum) for sf in primitives), "All should be SuperFrustum"

    print("\n✓ Initialization from slices test PASSED")
    return True


def test_initialization_from_voxels():
...
def test_optimization():
    ...
    # Optimize
    fitter = ResiduaFitter(learning_rate=0.01, optimization_steps=30)
...
def test_residual_computation():
    ...
    assert len(per_point_errors) == len(target_points), "Should have error for each point"
...
def test_primitive_addition():
    ...
    top_points = generate_cone_points(radius_bottom=1.5, radius_top=0.5, height=2.0, num_points=200)
    ...
    # Compute residual
    fitter = ResiduaFitter(max_primitives=5)
    total_error, per_point_errors = fitter.compute_residual_error(
        [primitive],
        target_points
    )
...
def test_full_fitting_workflow():
    ...
    # Generate cone shape
    target_points = generate_cone_points(radius_bottom=2.0, radius_top=0.5, height=3.0, num_points=1000)
    ...
    if len(history['errors']) > 1:
        assert history['errors'][-1] <= history['errors'][0], "Error should decrease or stay same"
