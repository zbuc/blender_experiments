...
def create_visual_hull_voxel_mesh(turntable_dir, resolution=128):
...
def test_sliceanalyzer_integration():
    ...
    clear_scene()
    visual_hull, bounds_min, bounds_max = create_visual_hull_voxel_mesh(turntable_dir, resolution=128)
    ...
    print(f"✓ Combined profile: {len(combined_profile)} samples, radius {min(radii):.3f}-{max(radii):.3f}")
    ...
    for i, sdata in enumerate(slice_data[:5]):
        center = sdata['center']
        scale = sdata['scale']
        print(f"  Slice {i}: center=({center.x:.2f}, {center.y:.2f}, {center.z:.2f}), radius={sdata['radius']:.3f}, scale=({scale.x:.2f}, {scale.y:.2f}, {scale.z:.2f})")
