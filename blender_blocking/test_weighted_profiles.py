...
def create_test_vase() -> bpy.types.Object:
...
def test_weighted_profiles() -> None:
    ...
    profiles = weighted_extractor.extract_weighted_profiles(
        vase, num_heights=20, bounds_min=bounds_min, bounds_max=bounds_max
    )
    ...
    for method in ["max", "mean", "geometric_mean"]:
        combined = weighted_extractor.combine_orthogonal_profiles(
            profiles["front"], profiles["side"], method=method
        )
