    ...
    for i in range(resolution):
        for j in range(resolution):
            for k in range(resolution):
                ...
                voxels_processed += 1
                if voxels_processed % (total_voxels // 10) == 0:
...
def test_groundtruth_multiprofile():
    ...
    # Join primitives
    if len(objects) > 0:
        final_mesh = MeshJoiner.join_with_boolean_union(objects, target_name="GroundTruthMultiProfile_Primitives")
